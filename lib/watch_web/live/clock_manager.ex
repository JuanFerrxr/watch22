defmodule WatchWeb.ClockManager do
  ...
  def handle_info(:"bottom-right", %{mode: Time, st3: Inactive} = state) do
  Process.send_after(self(), :wait_to_edit, 250)
  ...
  {:noreply, state |> Map.put(:st3, Inactive)}
  end


  def handle_info(:wait_to_edit, %{st3: Waiting} = state) do
  Process.send_after(self(), :stop_clock, 250)
  Process.send_after(self(), :editing_time, 250)
  ...
  def handle_info(:editing_time, %{count: count, show: show, st3: Editing, ui_pid: ui, time: time, selection: selection} = state) do
  count = count + 1
  if (count < 20) do
  Process.send_after(self(), :editing_time, 250)
  show = !show
  ...
  {:noreply, state |> Map.put(:mode, Editing) |> Map.put(:st, Stop)}
  end


  def handle_info(:resume_clock, %{st: Stop, time: time, ui_pid: ui} = state) do
